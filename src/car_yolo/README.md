...
`car_yolo` 是当前无人车侧的视觉检测包，负责在 RK3588 平台上直接读取本机摄像头图像，运行 RKNN YOLO11 推理，并将结果以 ROS2 话题形式发布。
...
当前更偏向**检测链路验证版**，已经足够对接“目标类别 + 图像框 + 时间戳 + 相机坐标系”这部分信息。
...
### 4.1 当前输入方式
当前节点不是订阅 ROS 图像话题，而是**直接打开本机 `/dev/videoX` 摄像头设备**。

这适合当前快速验证，但也意味着它暂时不是一个完全标准化的“图像话题消费者”。
...
4. **当前直接读本机摄像头，不是订阅 `/camera/image_raw`**
5. **当前 `camera_link` 只是消息里的 frame_id，TF 外参链还没有完整落地**
6. **没有与地图、SLAM、点云或深度信息做融合**
