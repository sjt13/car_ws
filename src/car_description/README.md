...
```text
odom -> base_footprint -> base_link -> {wheel links, imu_link, laser_link}
```
...
### 3.3 后续传感器融合扩展入口
当前已经有 `imu_link` 和 `laser_link`，后续如果要加相机，推荐继续在这个包里补：
- `camera_link`
...
当前这个包已经够支撑底盘、雷达、定位和导航联调，但如果后面要把视觉真正接到地图或导航里，建议优先补这几件事：

### 6.1 补相机挂点
建议后续在 URDF 中补：
- `camera_link`
- 必要时补 `camera_optical_frame`

### 6.2 明确相机外参
不要只在代码里口头说“相机大概装在前面”。要把：
- 前后偏移
- 左右偏移
- 高度
- 俯仰/偏航
